...
{
    class SqlServerCETailor : MTailor
    {
        #region Constructor

        public SqlServerCETailor() :
        ...
        public override void PerformBefore(DbObject node)
        {
            ...
            //Remove the default database owner
            if (node.HasNext && TailorUtil.IsDBO(node.Identifier))
            {
                node.Identifier.IsActive = false;
                //ReplaceTerm(node, new DbObject(new Identifier(node.Next.Identifier.ID)));
            }
            base.PerformBefore(node);
        }
