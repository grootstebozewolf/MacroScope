...
namespace MacroScope
{
    ...
    public class Stringifier : PassiveVisitor
    {
        ...
        public override void PerformAfter(DbObject node)
        {
            ...
            if (node.HasNext && node.Identifier.IsActive)
            {
                m_sql.Append('.');
            }

            if(node.IsParam && !node.HasNext)
            {
                m_sql.Append('$');
            }
        }
        ...
        public override void Perform(Identifier node)
        {
            ...
            if(node.IsActive) m_sql.Append(node.ID);
        }
        ...
        public override void PerformOnValues(InsertStatement node)
        {
            ...
            {
                m_sql.Append(@")");
                m_sql.Append(Environment.NewLine);
            }
            else
            {
                m_sql.Append(") VALUES (");    
        ...
        public override void PerformAfter(InsertStatement node)
        {
            ...
            if (node.HasDerivedTable) return;
            m_sql.Append(')');
        }
        ...
        public override void PerformBefore(DateTimeFunction node)
        {
            if (node.FunctionType == DateTimeFunction.DateAdd)
                m_sql.Append("DATEADD(");
            else if (node.FunctionType == DateTimeFunction.DateDiff)
                m_sql.Append("DATEDIFF(");
            else m_sql.Append("DATEPART(");

            if (node.Datepart == DateTimeUnit.Year) m_sql.Append("yy");
            if (node.Datepart == DateTimeUnit.Month) m_sql.Append("mm");
            if (node.Datepart == DateTimeUnit.Week) m_sql.Append("wk");
            if (node.Datepart == DateTimeUnit.Day) m_sql.Append("dd");
            if (node.Datepart == DateTimeUnit.Minute) m_sql.Append("mi");
            if (node.Datepart == DateTimeUnit.Second) m_sql.Append("ss");
            if (node.Datepart == DateTimeUnit.Weekday) m_sql.Append("dw");

            m_sql.Append(", ");
        }
        public override void PerformAfter(DateTimeFunction node)
        {
            m_sql.Append(')');
        }
        public override void PerformAfterNumber(DateTimeFunction node)
        {
            m_sql.Append(", ");
        }
        #endregion
